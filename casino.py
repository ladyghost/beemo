...
class Casino:
    ...
    @casino.command(name="leaderboardd", pass_context=True)
    ...
    async def _exchange_casino(self, ctx, currency: str, amount: int):
        ...
        if user.id in self.system["Players"]:
            if currency == "Chips":
                if amount > 0 and amount % credit_multiple == 0:
                    if amount <= self.system["Players"][user.id]["Chips"]:
                        ...
                        await self.bot.say("I have exchanged {} {} chips into {} credits.\nThank you for playing at ARDENT Casino.".format(amount, chips, str(int(credits)), casino_name))
                    else:
                ...
                if amount > 0 and amount % chip_multiple == 0:
                    if bank.can_spend(user, amount):
                        ...
                        dataIO.save_json(self.file_path, self.system)
                        await self.bot.say("I have exchanged {} credits for {} {} chips.\nEnjoy your time at ARDENT Casino!".format(amount, chip_amount, chips, casino_name))
                    else:
            ...
            await self.bot.say("You need a casino membership. To get one type `beemocasino join`.".format(casino_name))
    ...
    async def _stats_casino(self, ctx):
        ...
        if user.id in self.system["Players"]:
            ...
            await self.bot.say("You need a casino membership. To get one type `beemocasino join`.".format(casino_name, ctx.prefix))
    ...
    async def _toggle_casino(self, ctx):
        ...
        if self.system["System Config"]["Casino Open"]:
            ...
            dataIO.save_json(self.file_path, self.system)
            await self.bot.say("The ARDENT Casino is now closed.".format(casino_name))
        else:
            self.system["System Config"]["Casino Open"] = True
            dataIO.save_json(self.file_path, self.system)
            await self.bot.say("The ARDENT Casino is now open!".format(casino_name))
    ...
    async def _balance_setcasino(self, ctx, user: discord.Member, chips: int):
        ...
        if user.id in self.system["Players"]:
            ...
            await self.bot.say("{} needs a {} membership.".format(user.name, casino_name))
...
def check_files():
    system = {"System Config": {"Casino Name": "BeemoCasino",
                                "Casino Open": True,
                                "Chip Name": "Chip",
                                "Chip Rate": 1,
                                "Credit Rate": 1,
