...
class Shop:
    ...
    async def inventory(self, ctx):
        ...
        if user.id in self.players:
            ...
            await self.bot.say("You need to join the shop. Use the `beemoshop join` command.")
    ...
    async def _buy_shop(self, ctx, *, itemname):
        ...
        if self.config["Shop Open"]:
            if self.inventory_check(user.id):
                ...
                await self.bot.say("You need to join the {} shop to purchase items. Example: beemoshop join".format(shop_name))
        else:
    ...
    async def _gift_shop(self, ctx, user: discord.Member, *, itemname):
        ...
        if user.id in self.players:
            ...
            await self.bot.say("I cant find a user with that name." +
                               " Check to see if that user has joined {} shop. They need to type beemoshop join before they can recieve a gift.".format(shop_name))
    ...
    async def _notify_setshop(self, ctx):
        ...
        if self.config["Shop Notify"]:
            ...
            fileIO("data/shop/config.json", "save", self.config)
            await self.bot.say("Shop notifications are now ON!")
