...
class Economy:
    ...
    async def register(self, ctx):
        ...
        try:
            ...
            await self.bot.say("{} You already have a BeemoBank account.".format(user.mention))
    ...
    async def balance(self, ctx, user : discord.Member=None):
        ...
        if not user:
            ...
            try:
                ...
                await self.bot.say("{} You don't have a BeemoBank account."
    ...
    async def payday(self, ctx): # payyyydayyyyy
        ...
        if self.bank.account_exists(author):
            if id in self.payday_register[server.id]:
                ...
                if seconds  >= self.settings[server.id]["PAYDAY_TIME"]:
                    ...
                    await self.bot.say("{} You've received {} credits!".format(author.mention, str(self.settings[server.id]["PAYDAY_CREDITS"])))
                else:
                    await self.bot.say("{} I'm not your sugar daddy. Please wait {}.".format(author.mention, self.display_time(self.settings[server.id]["PAYDAY_TIME"] - seconds)))
            else:
                self.payday_register[server.id][id] = int(time.perf_counter())
                self.bank.deposit_credits(author, self.settings[server.id]["PAYDAY_CREDITS"])
                await self.bot.say("{} You've received {} credits!".format(author.mention, str(self.settings[server.id]["PAYDAY_CREDITS"])))
    ...
    async def _server_leaderboard(self, ctx, top : int=10):
        ...
        Defaults to top 10""" #need to work on this
    ...
    async def slot_machine(self, message, bid):
        ...
        if line[0] == ":two:" and line[1] == ":two:" and line[2] == ":six:":
            ...
            slotMsg = "{}{} Nothing! You've lost the bet, sorry. ".format(display_reels, message.author.mention)
    ...
    def display_time(self, seconds, granularity=2): # values
        intervals = (                               # source
